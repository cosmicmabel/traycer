...
>({
  from: { major: 3, minor: 0 },
  to: { major: 4, minor: 0 },
  // A v3.0 response without OpenClaw agents is a valid v4.0 response (purely
  // additive), and the request shape is identical - both upgrades are
  // identity.
  upgradeRequest: (request) => request,
  upgradeResponse: (response) => response,
...
>({
  ...
  to: { major: 3, minor: 0 },
  downgradeRequest: (request) => ({ ok: true, value: request }),
  // Drop OpenClaw agents so an already-shipped v3.0 client's strict decode
  // never sees one.
  downgradeResponse: (response) => ({
    ok: true,
...
>({
  ...
  to: { major: 2, minor: 0 },
  downgradeRequest: (request) => ({ ok: true, value: request }),
  // Drop post-v2.0 GUI harness agents (Amp AND OpenClaw) so an
  // already-shipped v2.0 client's strict decode never sees one.
  downgradeResponse: (response) => ({
    ok: true,
    value: listAgentsResponseSchemaV20.parse({
      ...response,
...
>({
  ...
  to: { major: 1, minor: 0 },
  downgradeRequest: (request) => ({ ok: true, value: request }),
  // Drop post-v1.0 GUI harness agents (ACP harnesses, Amp AND OpenClaw)
  // directly, so a v1.0 client's strict decode never sees any of them.
  downgradeResponse: (response) => ({
    ok: true,
    value: listAgentsResponseSchemaV10.parse({
      ...response,
