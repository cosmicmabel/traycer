...
/**
 * The click handler a legacy `<cic-*>` reference component runs, or `null`
 ...
 */
export type CicReferenceOpenHandler =
  ((event: MouseEvent<HTMLElement>) => void) | null;

/**
 * What `useCicReferenceOpenHandler` resolves in a single render pass:
 *
 ...
 */
export interface CicReferenceOpenState {
  readonly onOpen: CicReferenceOpenHandler;
 ...
 *   `focusArtifactId` for a node (D1); `cic-epic` carries `undefined`.
 */
type OpenTarget =
  | { readonly kind: "none" }
  | {
...
/**
 * Builds the open/navigate handler shared by every legacy `<cic-*>`
 ...
 *   `<cic-epic>`, which focuses the epic without an artifact.
 ...
 *   re-click after closing the tab re-opens it. `<cic-epic>` passes no
 ...
 * text.
 */
export function useCicReferenceOpenHandler(input: {
   ...
   * `false` (`cic-epic`), the reference focuses the epic with no node.
   */
  readonly requiresNode: boolean;
}): CicReferenceOpenState {
  ...
  const handler = useCallback(
    (event: MouseEvent<HTMLElement>): void => {
      ...
      switch (target.kind) {
        ...
        case "navigate":
          // `focusArtifactId` opens chat tiles too (D1); `cic-epic` passes
...
function resolveOpenTarget(input: {
  ...
  // CROSS epic (or same-epic epic-only focus): navigate + focus. `cic-epic`
