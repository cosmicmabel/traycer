...
// `CicMarkdown` bind to the mocked modules.
import { CicMarkdown } from "@/markdown/cic-markdown";
import { CicSpecReference } from "@/markdown/components/cic-spec-reference";
import { CicTicketReference } from "@/markdown/components/cic-ticket-reference";
import { CicChatReference } from "@/markdown/components/cic-chat-reference";
import { CicEpicReference } from "@/markdown/components/cic-epic-reference";
...
describe("legacy cic-* reference components", () => {
  ...
  it("same-epic chat opens a chat tile as a preview", () => {
    render(
      <CicChatReference data-epic-id={OPEN_EPIC_ID} data-chat-id="chat-1">
        Chat One
      </CicChatReference>,
    );
  ...
  it("cross-epic ticket navigates with focusArtifactId", () => {
    render(
      <CicTicketReference data-epic-id="epic-other" data-ticket-id="ticket-9">
        Ticket Nine
      </CicTicketReference>,
    );
  ...
  it("cross-epic chat reuses focusArtifactId (no focusChatId)", () => {
    render(
      <CicChatReference data-epic-id="epic-other" data-chat-id="chat-42">
        Chat Forty-Two
      </CicChatReference>,
    );
  ...
  it("cic-epic focuses the epic with no artifact id", () => {
    render(
      <CicEpicReference data-epic-id="epic-other">Some Epic</CicEpicReference>,
    );
  ...
  it("renders plain text when the embedded id is missing", () => {
    render(
      <CicSpecReference data-epic-id={OPEN_EPIC_ID}>Bare</CicSpecReference>,
    );
  ...
  it("renders plain text when the epic id is missing", () => {
    render(<CicChatReference data-chat-id="chat-1">No Epic</CicChatReference>);
  ...
  it("renders plain text when there is no open-epic session context", () => {
    mockHandle = null;
    render(
      <CicSpecReference data-epic-id={OPEN_EPIC_ID} data-spec-id="spec-1">
        Detached Spec
      </CicSpecReference>,
    );
  ...
  it("renders plain text when a same-epic node does not resolve", () => {
    render(
      <CicSpecReference data-epic-id={OPEN_EPIC_ID} data-spec-id="spec-missing">
        Missing Spec
      </CicSpecReference>,
    );
...
describe("same-epic spec/ticket chips are canvas drag sources", () => {
  ...
  it("a same-epic ticket chip registers a draggable, emits the payload, and wires the drag surface", () => {
    render(
      <CicTicketReference data-epic-id={OPEN_EPIC_ID} data-ticket-id="ticket-1">
        Ticket One
      </CicTicketReference>,
    );
...
describe("DEFAULT_COMPONENTS wires the legacy reference tags", () => {
  it("renders a clickable chip for a same-epic <cic-spec> tag", () => {
    renderMarkdown(
      `<cic-spec epicId="${OPEN_EPIC_ID}" specId="spec-1" title="Spec One">Spec One</cic-spec>`,
    );
