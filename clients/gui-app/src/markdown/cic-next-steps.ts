const CIC_NEXT_STEPS_OPEN_TAG = "<CIC_NEXT_STEPS>";
const CIC_NEXT_STEPS_CLOSE_TAG = "</CIC_NEXT_STEPS>";

export interface CicNextStepOption {
  readonly id: string;
  readonly prompt: string;
}

export type CicNextStepsPart =
  | {
...
const CLOSE_LINE_PATTERN = /^[\t ]*<\/CIC_NEXT_STEPS>[\t ]*$/;
const OPEN_AT_LINE_START_PATTERN = /^[\t ]*<CIC_NEXT_STEPS>[\t ]*/;
...
const completedParseCache = new Map<string, ReadonlyArray<CicNextStepsPart>>();

export function parseCicNextStepsMarkdown(
  markdown: string,
  isStreaming: boolean,
): ReadonlyArray<CicNextStepsPart> {
  if (!markdown.includes(CIC_NEXT_STEPS_OPEN_TAG)) {
    return [
      {
        kind: "markdown",
        id: "markdown:0",
  ...
  if (!isStreaming && markdown.length <= MAX_COMPLETED_PARSE_CACHE_CHARS) {
    ...
    const parsed = parseCicNextStepsMarkdownWithTags(markdown, false);
    completedParseCache.set(markdown, parsed);
    trimCompletedParseCache();
    return parsed;
  }

  return parseCicNextStepsMarkdownWithTags(markdown, isStreaming);
}

function parseCicNextStepsMarkdownWithTags(
  markdown: string,
  isStreaming: boolean,
): ReadonlyArray<CicNextStepsPart> {
...
export function repairCicNextStepsMarkdown(markdown: string): string {
  if (!markdown.includes(CIC_NEXT_STEPS_OPEN_TAG)) return markdown;
  const blocks = extractNextStepsBlocks(markdown, true);
  if (blocks.length === 0) return markdown;
  const last = blocks.at(-1);
  if (last === undefined || last.complete) return markdown;
  return `${markdown.trimEnd()}\n${CIC_NEXT_STEPS_CLOSE_TAG}`;
}
...
function pushMarkdownPart(parts: CicNextStepsPart[], markdown: string): void {
...
function parseNextStepsBlock(content: string): {
  ...
  const options: CicNextStepOption[] = [];
