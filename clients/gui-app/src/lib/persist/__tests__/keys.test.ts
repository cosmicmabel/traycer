...
describe("persist key builders — output-preserving against current source", () => {
  it("emits the current localStorage key for each of the 18 static stores", () => {
    ...
    expect(persistKey("onboarding")).toBe("cic-gui-app:onboarding");
    // Source: src/stores/command-palette/command-palette-store.ts
    expect(persistKey("command-palette")).toBe("cic-gui-app:command-palette");
    // Source: src/stores/composer/composer-draft-store.ts (plural divergence)
    expect(persistKey("composer-drafts")).toBe("cic-gui-app:composer-drafts");
    ...
    expect(persistKey("git-panel")).toBe("cic-gui-app:git-panel");
    ...
    expect(persistKey("left-panel")).toBe("cic-gui-app:left-panel");
    // Source: src/stores/file-tree/file-tree-store.ts
    expect(persistKey("file-tree")).toBe("cic-gui-app:file-tree");
    // Source: src/stores/home/history-search-store.ts
    expect(persistKey("history-search")).toBe("cic-gui-app:history-search");
    // Source: src/stores/home/landing-draft-store.ts (leaf `draft`, NOT
    // `landing-draft`).
    expect(persistKey("draft")).toBe("cic-gui-app:draft");
    ...
    expect(persistKey("keybindings")).toBe("cic-gui-app:keybindings");
    ...
    expect(persistKey("settings")).toBe("cic-gui-app:settings");
    // Source: src/stores/tabs/settings-section-store.ts (NOT a divergence)
    expect(persistKey("settings-section")).toBe("cic-gui-app:settings-section");
    ...
    expect(persistKey("tabs")).toBe("cic-gui-app:tabs");
  ...
  it("emits the current localStorage key for each of the 6 scoped stores", () => {
    ...
    expect(composerRunSettingsKey("a@b.com")).toBe(
      "cic-gui-app:composer-run-settings:a@b.com",
    );
    ...
    expect(composerHarnessMemoryKey("a@b.com")).toBe(
      "cic-gui-app:composer-harness-memory:a@b.com",
    );
    ...
    expect(worktreeIntentMemoryKey("a@b.com")).toBe(
      "cic-gui-app:worktree-intent-memory:a@b.com",
    );
    ...
    expect(worktreeIntentStagingKey("a@b.com")).toBe(
      "cic-gui-app:worktree-intent-staging:a@b.com",
    );
    // Source: src/stores/epics/canvas/store.ts (exported `persistKey`, bucketed
    // by userId).
    expect(epicCanvasKey(null)).toBe("cic-gui-app:epic-canvas:anon");
    expect(epicCanvasKey("u1")).toBe("cic-gui-app:epic-canvas:u1");
    // Source: src/stores/epics/open-epic/store.ts (local
    // `persistKey(epicId, userId)` emits `…:open-epic:{userBucket}:{epicId}`).
    expect(openEpicKey(null, "e1")).toBe("cic-gui-app:open-epic:anon:e1");
    expect(openEpicKey("u1", "e1")).toBe("cic-gui-app:open-epic:u1:e1");
  });
